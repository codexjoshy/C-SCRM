...
import stripBom from 'strip-bom'; // Needed because fs has no encoding, like utf-8-sig in python, for files with BOM (byte order mark)
...
loadSessionData = () => {
  ...
  resourcePaths.forEach((resource) => {
    loadCSVFileContents(
      dataPath + '/' + resource.path,
      resource.type,
      (data) => {
        data.forEach(
          (d) =>
            d._cscrm_active && (d._cscrm_active = parseInt(d._cscrm_active))
        );
        return data;
      }
    );
  ...
  if (type === 'resources') {
    resourcePaths.forEach((resource) => {
      if (sessionData[resource.type].length > 0) {
        try {
          ...
          writeFile(dataPath + '/' + resource.path, csv, (err) => {
            if (!err) {
    ...
    responsePaths.forEach((path) => {
      if (Object.keys(sessionData[path.type]).length > 0) {
        ...
        writeFile(dataPath + '/' + path.path, data, (err) => {
          if (!err) {
